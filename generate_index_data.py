...
def build_index(pages):
    ...
    for i, page in enumerate(pages, 1):
        for c in extract_candidates(page):
            if len(c.split()) < 2:
                continue
...
def compare_indexes(raw, curated):
    ...
    for k in common:
        r = raw[k]   # generated
        c = curated[k]  # found / edited

        diffs = []

        if r.get("normalized") != c.get("normalized"):
            diffs.append(
                f"normalized: '{r.get('normalized')}' → '{c.get('normalized')}'"
            )

        if r.get("type") != c.get("type"):
            diffs.append(
                f"type: '{r.get('type')}' → '{c.get('type')}'"
            )

        if r.get("action") != c.get("action"):
            diffs.append(
                f"action: '{r.get('action')}' → '{c.get('action')}'"
            )
...
def print_diff_report(raw, curated):
    ...
    if added:
        print("➕ New entries (in raw, not in curated):")
    ...
    if removed:
        print("\n➖ Entries only in curated (not in raw):")
        for k in sorted(removed):
            print(f"   - {k}")
    else:
        print("\n➖ No curated-only entries")
...
def initialize_curated_file(raw_data):
    ...
    if curated_path.exists():
        print(f"ℹ️ {CURATED_JSON} already exists — comparing with new raw...")
    ...
    print(f"✅ Created {CURATED_JSON} (initial copy of raw index)")
...
def main():
    ...
    print("\n👉 Next: edit index_curated.json and run build script.")
